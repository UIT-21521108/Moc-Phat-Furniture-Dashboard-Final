    ...
    try:
        ...
        for c in cols_text:
            if c not in df.columns:
                ...
                df[c] = df[c].fillna("UNKNOWN").astype(str).str.upper().str.strip()

        df['sl'] = pd.to_numeric(df.get('sl'), errors='coerce').fillna(0)
...
if sel_cust:
    df = df[df['khach_hang'].isin(sel_cust)]
if df.empty:
...
with tab1:
    ...
    fig_trend.add_trace(go.Scatter(
        x=ts_data['ym'], y=ts_data['sl'], mode='lines', name='Sản lượng',
    ...
    fig_trend.add_trace(go.Scatter(
        x=ts_data['ym'], y=ts_data['ma3'], mode='lines', name='TB 3 tháng',
    ...
    with c1_1:
        ...
        fig_cust = px.bar(top_cust, x='sl', y='khach_hang', orientation='h',
                          text_auto='.2s', color='khach_hang',
    ...
    with c1_2:
        ...
        fig_donut = px.pie(grp_color, values='sl', names='nhom_mau', hole=0.6,
    ...
    with c1_3:
        ...
        if treemap_data['sl'].sum() > 0:
            treemap_data = treemap_data[treemap_data['sl'] > treemap_data['sl'].sum()*0.01]
        fig_tree = px.treemap(treemap_data, path=['nhom_mau', 'mau_son'], values='sl',
                              color='nhom_mau', color_discrete_sequence=px.colors.qualitative.Set2)
        st.markdown('<div class="chart-box">', unsafe_allow_html=True)
    ...
    with c1_4:
        ...
        fig_q = px.bar(q_data, x='quarter', y='sl', color='year',
...
with tab2:
    ...
    if not base_2025.empty:
        ...
        combined_forecast = pd.concat([monthly_2025, monthly_2026], ignore_index=True)
        ...
        st.info("Chưa có dữ liệu 2025 để lập kế hoạch 2026.")
...
with tab3:
    ...
    with c_sku:
        ...
        fig_sku = px.bar(top_sku, x='sl', y='ma_hang', orientation='h',
                         text_auto='.2s', color_discrete_sequence=[PRIMARY_NEON])
        st.markdown('<div class="chart-box">', unsafe_allow_html=True)
    ...
    with c_usb:
        ...
        fig_usb = px.bar(usb_trend, x='year', y='sl', color='is_usb_clean', barmode='group',
                         color_discrete_map={'Có USB': '#FFA726', 'Không USB': '#333', 'N/A': '#9E9E9E'})
        st.markdown('<div class="chart-box">', unsafe_allow_html=True)
...
with tab5:
    ...
    with c5_1:
        ...
        fig_p.add_trace(go.Scatter(x=pareto['khach_hang'], y=pareto['perc'], name='% Tích lũy', yaxis='y2',
                                   line=dict(color='#FF5252', width=2)))
        fig_p.update_layout(yaxis2=dict(overlaying='y', side='right', range=[0, 110]), showlegend=False)
        # vline 80%
        idx_80 = pareto.index[(pareto['perc'] >= 80)]
        if len(idx_80) > 0:
            fig_p.add_vline(x=int(idx_80.min()), line_dash="dash", line_color="#FF5252", opacity=0.6)
    ...
    with c5_2:
        ...
        growth = ((v_c - v_p) / v_p * 100).replace([np.inf, -np.inf], np.nan).fillna(0).sort_values(ascending=False).reset_index()
